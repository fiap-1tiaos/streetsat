...
def simple_graph():
    segments = [
        {"road": "SP-330", "km_start": 0, "km_end": 10, "lat_start": -23.0, "lon_start": -46.0,
         "lat_end": -23.1, "lon_end": -46.0, "risk_score": 0},
        {"road": "SP-330", "km_start": 10, "km_end": 20, "lat_start": -23.1, "lon_start": -46.0,
         "lat_end": -23.2, "lon_end": -46.0, "risk_score": 2},
        {"road": "SP-330", "km_start": 20, "km_end": 30, "lat_start": -23.2, "lon_start": -46.0,
...
def test_edge_weights_positive(simple_graph):
    for _u, _v, data in simple_graph.edges(data=True):
        assert data["weight"] > 0


def test_find_nearest_node(simple_graph):
    node = find_nearest_node(simple_graph, "SP-330", 5.0)
    assert node is not None
    assert node[0] == "SP-330"


def test_route_found(simple_graph):
    origin = find_nearest_node(simple_graph, "SP-330", 0)
    dest = find_nearest_node(simple_graph, "SP-330", 30)
