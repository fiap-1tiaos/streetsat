...
export default function MapPage() {
  ...
  const filtered = occurrences.filter(
    (o: Occurrence) => o.risk_score >= minScore && (!brFilter || String(o.br).includes(brFilter))
  )

  return (
    <div style={{ position: 'relative', height: 'calc(100dvh - 64px - 3rem)', borderRadius: '12px', overflow: 'hidden' }}>
      <RiskMap occurrences={filtered} onSelect={setSelected} height="100%" />
          ...
          >
            ...
            {selected.narrativa && (
              <div style={{ marginTop: '1rem', padding: '0.75rem', background: 'rgba(0,212,255,0.04)', border: '1px solid rgba(0,212,255,0.08)', borderRadius: '8px' }}>
                ...
                <p style={{ fontSize: '0.8rem', color: '#94a3b8', lineHeight: 1.6, margin: 0, fontStyle: 'italic' }}>{selected.narrativa}</p>
              </div>
            )}
          </motion.div>
        )}
