...
def _get_or_build_graph():
    ...
    if _graph is None:
        segments = [
            {"road": "SP-330", "km_start": i * 10, "km_end": (i + 1) * 10,
...
async def optimize_route(body: dict):
    ...
    origin_road = str(origin_data.get("road", "SP-330"))
    origin_km = float(origin_data.get("km", 0))
    dest_road = str(dest_data.get("road", "SP-330"))
    dest_km = float(dest_data.get("km", 100))

    G = _get_or_build_graph()

    origin = find_nearest_node(G, origin_road, origin_km)
    destination = find_nearest_node(G, dest_road, dest_km)
    ...
    try:
        ...
        return {
            "origin": {"road": origin[0], "km": origin[1]},
            "destination": {"road": destination[0], "km": destination[1]},
            "safe_route": {
                "nodes": [{"road": n[0], "km": n[1]} for n in safe.nodes],
